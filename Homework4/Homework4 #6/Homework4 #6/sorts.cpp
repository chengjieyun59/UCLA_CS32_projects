// This file creates a randomly ordered array, sorts it in a few ways, and reports on the elapsed times
...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
    ...
    for (size_t i = 1; i < s.size(); i++) // i iterates through the unsorted elements
    {
        Sensor temp = s[i]; // store temp
        size_t j = i;
        
        // for each additional i, move all the elements that should come after i to the right.
        for (j = i; j > 0 && comp(temp, s[j-1]); j--)
        {
            s[j] = s[j-1];
        }
        s[j] = temp; // insert
